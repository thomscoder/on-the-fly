...
import { createTerminal, listenToElementChanges, openEditorArea, startShell } from './actions/actions';
...
window.addEventListener('load', async () => {
  ...
  if (editorEl) {
    ...
    monacoEditor.getModel()!.setValue(files.example.directory["index.js"].file.contents);
    listenToElementChanges('example', webcontainerInstance, monacoEditor);
  }

  // open draggable editor clicking button
  if (buttonEl && editorEl) {openEditorArea(buttonEl, editorEl)}
  ...
  startShell(webcontainerInstance, iframeEl, {writeToElement: terminal, additionalElement: previewEl});
});
  ...
  <div class="container">
    ...
    <div class="sub-container">
      ...
      <div class="preview">
        ...
        <button class="edit-button">index.js</button>
      </div>
    </div>
  </div>
`;
